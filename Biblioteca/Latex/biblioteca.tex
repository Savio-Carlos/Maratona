...
\begin{lstlisting}
f20 si = vetor se diz se aquela posicao esta valida/funcionanando
483 on = arvore que conta quantos estao ligados
b7d valid = arvore que conta quantos sao validos naquele intervalo (exclusivo de um problema la)
007 toda operacao ele calcula os ligados com base em quantos tem valido naquele intervalo - os desligados, ja que ta sempre invertendo
c4c */
99a int n, tree[4*MAX], on[4*MAX], valid[4*MAX], si[MAX];
1d3 bool lazy[4*MAX];
...
435     build(node*2, l, m);
4c9     build(node*2+1, m+1, r);
931     valid[node] = valid[node*2] + valid[node*2+1];
be9     on[node] = on[node*2] + on[node*2+1];
0f7 }
885 void flip(int node){
890     on[node] = valid[node] - on[node];
006     lazy[node] = !lazy[node];
c10 }

7ec void prop(int node){
996     if (lazy[node]){
1c2         flip(node*2);
f5e         flip(node*2+1);
48c         lazy[node] = false;
...
938 void update(int node, int l, int r, int a, int b){
1dd     if(b < l or r < a) return;
02b     if(a <= l && r <= b){
f7b         flip(node);
505         return;
...
dcf     prop(node);
ee4     int m = (l+r)/2;
325     update(2*node, l, m, a, b);
cf4     update(2*node+1, m+1, r, a, b);
be9     on[node] = on[node*2] + on[node*2+1];
031 }
\end{lstlisting}
...
be5 struct point {
e91     int x, y;
f3e     point(int x_ = 0, int y_ = 0) : x(x_), y(y_) {}
...
ac5     point operator + (const point& p) const { return point(x + p.x, y + p.y); }
6d4     point operator - (const point& p) const { return point(x - p.x, y - p.y); }
477     template<typename U>
563     point operator * (const U c) const { return point(x * c, y * c); }
522     auto operator * (const point& p) const { return (int)x * p.x + (int)y * p.y; }
5db     auto operator ^ (const point& p) const { return (int)x * p.y - (int)y * p.x; }
    
d70     friend istream& operator >> (istream& in, point& p) { return in >> p.x >> p.y; }
322     friend ostream& operator << (ostream& os, const point& p) { return os << "(" << p.x << ", " << p.y << ")"; }
252 };
...
1f0 bool perpendicular(point v, point w){
350     return (v*w == 0);
3e1 }

//ccw, 0 colineares, -1 r a direita, 1 r a esquerda
7bf int orientation(point p, point q, point r){
c5b     int val = ((q-p) ^ (r-p));
db6     if (!val) return 0;
...
5f0 bool onAngle (point p, point q, point r, point g){
...
533 int quad(point p) { 
dbb 	return (p.x<0)^3*(p.y<0);
c46 }

//dist quadrada de dois pontos
4c6 int dist2(point p, point q){
f24     return sq(p.x - q.x) + sq(p.y - q.y);
507 }

// 2 * area com sinal
45e int sarea2(point p, point q, point r) { 
586 	return (q-p)^(r-q);
73b }

// 2 * area do poligono
c1f int polarea2(vector<point> v) { 
7c9 	int ret = 0;
c6e 	for (int i = 0; i < v.size(); i++)
de6 		ret += sarea2(point(0, 0), v[i], v[(i + 1) % v.size()]);
d03 	return abs(ret);
4a3 }

//checa se o poligono e convexo
a72 bool isConvex(vector<point> p) {
...
a6f bool half(point p) { // true if in blue half
...
fa6 void polarSort(vector<point> &v) {
621     sort(v.begin(), v.end(), [](point v, point w) { 
...
9e0 int segpoints(line r) { 
508 	return 1 + gcd(abs(r.p.x - r.q.x), abs(r.p.y - r.q.y));
a3e }

c1f int polarea2(vector<point> v){ // 2 * area do poligono
7c9 	int ret = 0;
f14 	for (int i = 0; i < v.size(); i++){
de6 		ret += sarea2(point(0, 0), v[i], v[(i + 1) % v.size()]);
3fe     }
...
c4d using int64 = long long;
f26 const int MAXN = 200000 + 5;
c58 const int64 NEG_INF = LLONG_MIN / 4;
 
3b9 int n, q;
ea6 vector<int> adj[MAXN];
be6 int64 val[MAXN];
 
1a0 int parentArr[MAXN], depthArr[MAXN], heavy[MAXN], head[MAXN], pos[MAXN], szArr[MAXN];
2b0 int curPos;
2e5 vector<int64> baseArr;
...
7b2         N = 1;
ca2         while (N < n) N <<= 1;
9f7         st.assign(2*N, NEG_INF);
b1d     }
242     void build(const vector<int64>& a) {
b3b         init((int)a.size());
d50         for (int i = 0; i < (int)a.size(); ++i) st[N+i] = a[i];
d0b         for (int i = N-1; i >= 1; --i) st[i] = max(st[2*i], st[2*i+1]);
b88     }
5b5     void update(int idx, int64 value) {
c94         int p = N + idx;
f6b         st[p] = value;
8b8         p >>= 1;
d08         while (p) {
629             st[p] = max(st[2*p], st[2*p+1]);
8b8             p >>= 1;
f03         }
007     }
889     int64 queryRange(int l, int r) { // inclusive l,r
4df         if (l > r) return NEG_INF;
a29         l += N; r += N;
cc5         int64 res = NEG_INF;
3d5         while (l <= r) {
762             if (l & 1) res = max(res, st[l++]);
059             if (!(r & 1)) res = max(res, st[r--]);
f69             l >>= 1; r >>= 1;
7a7         }
b50         return res;
500     }
...
a5b int dfs(int u, int p) {
81a     parentArr[u] = p;
d21     depthArr[u] = (p == -1 ? 0 : depthArr[p] + 1);
650     szArr[u] = 1;
8b7     int maxSub = 0;
032     heavy[u] = -1;
...
c88 void decompose(int u, int h) {
36a     head[u] = h;
19f     pos[u] = curPos++;
b8b     baseArr[pos[u]] = val[u];
eee     if (heavy[u] != -1) decompose(heavy[u], h);
876     for (int v : adj[u]) if (v != parentArr[u] && v != heavy[u]) {
e27         decompose(v, v);
0b4     }
950 }
 
249 int64 query_path(int a, int b) {
cc5     int64 res = NEG_INF;
2da     while (head[a] != head[b]) {
6d0         if (depthArr[head[a]] < depthArr[head[b]]) swap(a,b);
64e         int ha = head[a];
611         res = max(res, seg.queryRange(pos[ha], pos[a]));
0af         a = parentArr[ha];
77b     }
bb0     if (depthArr[a] < depthArr[b]) swap(a,b);
424     res = max(res, seg.queryRange(pos[b], pos[a]));
b50     return res;
ac9 }
...
b1d         adj[i].clear();
4c8     }
96b     for (int i = 0; i < n-1; ++i) {
417         int a,b; cin >> a >> b; --a; --b;
fab         adj[a].push_back(b);
c87         adj[b].push_back(a);
1b6     }
c91     dfs(0, -1);
5ad     curPos = 0;
6ea     baseArr.assign(n, NEG_INF);
70b     decompose(0, 0);
56c     seg.build(baseArr);
     
1f4     while (q--) {
452         int type; cin >> type;
821         if (type == 1) {
54a             int s; int64 x; cin >> s >> x; --s;
aa9             val[s] = x;
95e             seg.update(pos[s], x);
10a         } else {
417             int a,b; cin >> a >> b; --a; --b;
949             int64 ans = query_path(a,b);
cb9             cout << ans << '\n';
192         }
21b     }
bb3     return 0;
...
6e6 r * ((fastExpo((r/k), n)) - (fastExpo((ld)(r-1)/k, n)));
\end{lstlisting}
