...
namespace SegTree {
	...
	int update(int a, int b, int x, int node=1, int l=0, int r=n-1) {
		prop(node, l, r);
		if (a <= l and r <= b) {
            if (lazy[node] < x) lazy[node] = x;
			prop(node, l, r);
			return tree[node];
		}
		if (b < l or r < a) return tree[node];
		int m = (l+r)/2;
		return tree[node] = max(update(a, b, x, 2*node, l, m), update(a, b, x, 2*node+1, m+1, r));
	}
}
...
signed main (){
    ...
    while(q--){
        int x, y;
        cin >> x >> y;
        SegTree::update(0, x-1, y);
        int tras = SegTree::prop(0,x-1);
        debug(SegTree::ans);
        SegTree::ans = 0;
        for (int i = 0; i < 2*n; i++){
            debug(i, SegTree::tree[i], SegTree::lazy[i]);
        }
    }
}