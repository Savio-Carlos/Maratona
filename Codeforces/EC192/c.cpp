...
void solve(){
    ...
    vector<int> a(n);
    cin >> a;
    vector<int> cnt(n+1);//contagem de cada numero
    vector<int> cntcnt(n+1);//contagem de quantas contagems tem
    int m = 0;
    for (auto u : a){
        if (cnt[u] == 0) m++;
        cnt[u]++;
    ...
    vector<int> seq;
    for (int i = 1; i < cntcnt.size(); i++){
        seq.push_back(m);
        if (cntcnt[i]) m-=cntcnt[i];
        if (m == 0) break;
    }
    debug(seq);
    //use seq to reach k, if we add, we stay in place, if we remove, we move forward in seq
    //count number of ways of reaching K
    int nn = n;
    int ways = (nn == k);
    for (int i = seq.size()-1; i >= 0; i--){
        debug(seq[i], nn);
        if ((k - nn) % seq[i] == 0) ways++;
        nn -= seq[i];
    }   
    cout << ways << endl;
