...
void solve(){
    ...
    vector<int>a(n);
    cin >> a;
    vector<vector<int>> pfx(n, vector<int>(4, 0));
    vector<int> cnt(4,0);
    vector<int> i1;
    vector<int> i2;
    for (int i = 0; i < n-1; i++){
        cnt[a[i]]++;
        pfx[i] = cnt;
        if (cnt[1] == cnt[2] + cnt[3] or (cnt[2] == 0 and cnt[3] == 0)) i1.push_back(i);
        if (cnt[1] + cnt[2] >= cnt[3]) i2.push_back(i);
    }
    bool pos = false;
    debug(i1,i2);
    for (auto u : i1){
        for (int j = u+1; j < n-1; j++){
            ...
            auto c2 = pfx[j];
            debug(c1,c2);
            if ((c2[2] + c2[1]) - (c1[1] + c1[2]) >= (c2[3] - c1[3])){
                pos = true;
                cout << "YES" << endl;
                return;
            } 
