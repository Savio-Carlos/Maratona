...
//if ai = 0 and bi = 1 it doesnt matter becauuse we can join with one op where ai = 1 and bi = 0
//count number of ai = 1 and bi = 0, if odd then 1 op, if even then split in 2 op
//if sum of a is zero then we can never make an operation
//we can never make a = b if b is all 1, because we need at least one 1 in a to make the sum odd,
//and this will be flipped to 0
void solve(){
    int n; 
    cin >> n;
    vector<int> a(n), b(n);
    ...
    if(sumA == 0 or sumB == n) cout << -1 << endl;
