...
void solve(){
    ...
    int m = b.size();
    auto comp = [&](int v) {
        return (int)(lower_bound(all(b), v) - b.begin());
    };
    ...
    for (int i = 0; i <= n; i++) {
        ...
        if (!(i&1)) bit1.update(c, 1);
        else ans += bit1.pref(c-1);
        
        if (i&1) bit2.update(c, 1);
        else ans += bit2.query(c+1, m-1);
    }
