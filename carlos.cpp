...
namespace dbg {
    ...
    // Detect iterable containers (have value_type), excluding string/string_view
    ...
    // Leaf overloads first — no recursion needed
    ...
    // Generic fallback for non-containers (primitives, user structs with operator<<)
    template<typename T, enable_if_t<!is_container<T>::value, int> = 0>
    ostream& print_atom(ostream& os, const T& x) { return os << x; }

    // Pair and tuple — depend only on leaf/fallback above, defined before container
    ...
    // stack / queue / priority_queue — copy and drain
    ...
    // Container — defined LAST so all specific overloads above are visible at this
    // template's point of definition, making them candidates for the recursive call
    ...
    // operator<< wrappers so cerr << vec works outside debug()
    template<typename C, enable_if_t<is_container<C>::value, int> = 0>
