...
signed main() {
    ...
    // Calculate distance via ring k
    // Distance = R/N * (|ay - k| + |by - k| + k * pi * |bx - ax| / M)
    
    int dx = abs(bx - ax);
    
    // Try k = 0 (go through center)
    ld dist_via_0 = (R / N) * (ay + by);
    
    // Try k = min(ay, by) (optimal intermediate ring)
    int k = min(ay, by);
    ld dist_via_k = (R / N) * (abs(ay - by) + k * pi * dx / M);
    
    ld ans = min(dist_via_0, dist_via_k);
